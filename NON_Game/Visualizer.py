...
class MarkerInlet(Inlet):
    ...
    def __init__(self, info: pylsl.StreamInfo):
        super().__init__(info)
        self.markers = []  # Keep track of added markers

    def pull_and_plot(self, plot_time, plt):
        # Remove old markers that are not in the current plot_time range
        self.markers = [marker for marker in self.markers if marker.value() > plot_time]
        for marker in self.markers:
            plt.removeItem(marker)  # Remove old markers from the plot

        strings, timestamps = self.inlet.pull_chunk(0)
        if timestamps:
            for string, ts in zip(strings, timestamps):
                event_description = string[0]  # The description of the event, such as "Object destroyed"
                pen = None  # Default pen color
                if "destroyed" in event_description:  # Check for object destruction events
                    pen = pg.mkPen('r')  # Use red color for object destruction events
                elif "flash" in event_description:  # Check for light flash events
                    pen = pg.mkPen('y')  # Use yellow color for light flash events
                marker = pg.InfiniteLine(ts, angle=90, movable=False, pen=pen)
                plt.addItem(marker)
                self.markers.append(marker)  # Add new markers to the tracking list
